    ...
    <div className="bg-secondary/20 border-b border-border" onClick={(e) => e.stopPropagation()}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                  ...
                  <Area
                    ...
                    stroke={chartColor}
                    strokeWidth={1.5}
                    fill={`url(#fill-${holding.id})`}
                    dot={false}
                    activeDot={{ r: 3, fill: chartColor, stroke: "hsl(240, 16%, 8%)", strokeWidth: 2 }}
        ...
        <div className="sm:flex-1 min-w-0 flex flex-col gap-3">
          <div className="grid grid-cols-3 gap-x-4 gap-y-2">
            ...
            <Stat
              label="Div Yield"
              value={
                financials?.dividendYieldIndicatedAnnual != null
                  ? financials.dividendYieldIndicatedAnnual.toFixed(2) + "%"
                  : "—"
              }
            />
          </div>
          ...
          {w52Range > 0 && (
            <div className="space-y-0.5">
              ...
              <div className="relative h-1.5 rounded-full bg-secondary">
                <div
                  className="absolute top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-primary border border-card"
                  style={{ left: `${Math.min(Math.max(w52Pct, 2), 98)}%` }}
                />
              </div>
            </div>
          )}
          ...
          {financials?.dividendYieldIndicatedAnnual != null &&
            financials.dividendYieldIndicatedAnnual > 0 &&
            (() => {
              const safety = calcDivSafety(
                financials.dividendYieldIndicatedAnnual!,
                financials.payoutRatioAnnual ?? null,
                financials.dividendGrowthRate5Y ?? null,
              );
                  ...
                  <div className="grid grid-cols-3 gap-1 text-[10px]">
                    <div>
                      ...
                      <span className="font-mono text-foreground">
                        {financials.payoutRatioAnnual?.toFixed(0) ?? "—"}%
                      </span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="font-mono text-foreground">{safety.growthLabel}</span>
                    </div>
                  </div>
              ...
              news.slice(0, 3).map((n) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-foreground truncate group-hover/news:text-primary transition-colors">
                      {n.headline}
                    </p>
      ...
      <div className="px-3 sm:px-5 pb-4">
        <TaxLotsPanel
          ...
          onRequestRemoveHolding={onDelete}
        />
      </div>
    </div>
  );
